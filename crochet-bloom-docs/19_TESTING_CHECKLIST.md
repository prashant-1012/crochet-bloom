...
## Functional
- [ ] Add to cart from Featured Products and `/shop` both work and
      merge quantities correctly for the same product
- [ ] `/shop` category grouping/filtering (Bouquets, Keychains,
...
- [ ] Floating WhatsApp button opens a general-inquiry/custom-order
      chat, unrelated to cart contents
- [ ] All nav links route correctly; mobile menu opens/closes and
      closes on route change
- [ ] Contact form submission opens WhatsApp with the form content
      pre-filled
...
- [ ] Touch targets ≥44px on mobile, including category filter chips
...
- [ ] Color contrast spot-checked against
      [07_COLOR_SYSTEM.md](./07_COLOR_SYSTEM.md)'s table — especially
      that bright Bloom/Yarn are never used as body-sized text or a
      solid button fill
- [ ] `prefers-reduced-motion` respected (test via OS/browser setting)

## Performance / SEO
- [ ] Lighthouse run on Home, `/shop`, `/about` — target ≥90 across
