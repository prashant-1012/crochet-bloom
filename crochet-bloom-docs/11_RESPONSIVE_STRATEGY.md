...
traffic is mobile (Instagram referrals, occasion-driven quick
purchases).
...
- Product/testimonial grids: 1 column mobile → 2 columns `sm`/`md` →
  3–4 columns `lg`+. Given the catalog spans four categories
  (Bouquets, Keychains, Hampers, Decor), the `/shop` grid should also
  support a category filter/grouping that collapses sensibly on
  mobile (e.g. horizontally scrollable category chips rather than a
  sidebar).
...
- Touch targets: minimum 44×44px for any tappable control (buttons,
  quantity steppers, nav links, category filter chips) regardless of
  breakpoint.
