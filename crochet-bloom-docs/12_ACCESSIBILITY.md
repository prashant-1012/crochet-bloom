...
- Every `next/image` usage requires a meaningful `alt`. Purely
  decorative images (thread swirls, blobs, background flourishes) get
  `alt=""` so screen readers skip them.
- Product images: `alt` describes the product ("Twirling Tulips
  crochet bouquet"), not generic "flower image."
...
- All text/background pairings validated against
  ...
  shipping a component — in particular, never use bright Bloom or
  bright Yarn as body-sized text or a solid button fill; only the
  darkened variants pass AA.
